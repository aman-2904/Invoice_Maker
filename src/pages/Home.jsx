...
function Home() {
    ...
    // DEBUG: Check connection on load
    useEffect(() => {
            ...
            try {
                ...
                if (error) {
                    ...
                    if (error.code === 'PGRST301') {
                        alert('Error: JWT expired or invalid keys. Check .env file.');
                    } else if (error.message.includes('relation "public.invoices" does not exist')) {
                        alert('CRITICAL: Database table "invoices" NOT FOUND. You MUST run the SQL command in Supabase Dashboard.');
                    } else {
                        alert('Supabase Error: ' + error.message);
                    }
                } else {
                    console.log('Supabase Connected! Invoices count:', count);
                }
            } catch (err) {
    ...
    const saveInvoice = async () => {
        ...
        try {
            // Include settings in the saved data
            const payload = {
                ...formData,
                settings: {
                    gstRate,
            ...
            // Optional: Redirect to Admin or Reset?
            // navigate('/admin'); 
        } catch (error) {
